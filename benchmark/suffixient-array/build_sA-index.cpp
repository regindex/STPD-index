...
void help(){
    ...
    "-b <arg>    Tabulation string length.               (Def. 15)" << std::endl <<
...
int main(int argc, char* argv[])
{
    ...
    std::string inputPath, outputPath, oracleVariant = "RLZ";
    ...
    int opt;
    while ((opt = getopt(argc, argv, "hi:o:t:l:b:k")) != -1)
    {
        switch (opt){
            case 'h':
                help();
    ...
    std::string suffixient_set = inputPath + ".suff";
    if( not std::filesystem::exists(suffixient_set) )
    {
        std::cout << "\n[INFO] Constructing and storing the Suffixient array index (sA-index)" 
                  << " for " << inputPath << std::endl;

        { // compute the path decomposition
            std::cout << "[STEP 0] Computing the Suffixient set sampling..." << "\n" << std::endl;

            std::filesystem::path exe_path = std::filesystem::canonical(argv[0]);
            std::filesystem::path exe_dir = exe_path.parent_path();

            std::string command = std::string(exe_dir) +
                                  "/build_suffixient -i " + inputPath + " -o " + inputPath + ".suff"; 
            int result = std::system(command.c_str());
            if (result != 0) {
                std::cerr << "Error while computing the suffixient set..." << std::endl;
                exit(1);
            }
        }
    }
    else
        std::cout << "\n[INFO] Suffix set found! Proceeding to index construction\n" << std::endl;

    { // select the correct index variant
        std::visit([&](auto& idx) {
            // compute the index
            idx.build(inputPath,inputPath+".suff",refLen,tabLen);
            // store the index
    ...
    { // delete temporary files
        std::remove(std::string(inputPath+".suff").c_str()); 
    }
