...
{  
  ...
  for(int i=0;i<dwords-1;i++)
    assert(eos[i]<eos[i+1]); // important since we'll use binary search on eso[]
  ...
  // in the SA of dict skip suffixes starting with 0x0 0x1 and the last w 0x2's
  // but not the first 0x2 which was d[0] (but we overwrote it with 0)
  // that word (0x2T[0]T[1]...) has the role of EOF for T so it generates
  // the BWT entry T[n-1], but not a suffix array entry  
  assert(sa[dwords+arg.w+1]==0); // start with the word 0x2T[0]T[1]....
  for(long i=dwords+arg.w+1; i< dsize; i=next ) { 
    ...
    if(sa[i]==0 || d[sa[i]-1]==EndOfWord) {
      full_words++;
      // the first word appears once 
      // ilist[j] scans all the BWT positions containing that full word 
      for(long j=istart[seqid];j<istart[seqid+1];j++) {
        ...
        if(arg.SA) { // full SA requested 
          ...
          else assert(j==1); // the first word in the parse is the 2nd lex smaller and does not correspond 
                             // to a SA entry because it starts with Dollar which is not in T[] 
        }
    ...
    while(next<dsize && lcp[next]>=suffixLen) {
      assert(lcp[next]==suffixLen);  // the lcp cannot be greater than suffixLen: eos char are different!
      ...
      else die("Parsing property violated: trigger string undetected");
    }
...
// Note, we use gsacak rather than gsais, since according to the authors
// (10.1016/j.tcs.2017.03.039) the former is faster and uses less memory
