...
template <util::AllocType AT = util::AllocType::MALLOC> class SimpleSelectZeroHalf {
  ...
  public:
	...
	uint64_t serialize(std::ostream& out)
	{
		...
		out.write((char*)&inventory_size, sizeof(inventory_size));
		out.write((char*)&num_zeros, sizeof(num_zeros));

		w_bytes += sizeof(num_words) + sizeof(inventory_size) + sizeof(num_zeros);

		w_bytes += inventory.serialize(out);

		return w_bytes;
	}

	void load(std::istream& in, uint64_t *bits_)
	{
		bits = bits_;

		in.read((char*)&num_words, sizeof(num_words));
		in.read((char*)&inventory_size, sizeof(inventory_size));
		in.read((char*)&num_zeros, sizeof(num_zeros));
