...
class Rank {
  public:
	 ...
	 */
	virtual uint64_t rank1(std::size_t pos) const = 0;
	 ...
	 */
	uint64_t rank1(std::size_t from, std::size_t to) const { return rank1(to) - rank1(from); }
	 ...
	 */
	virtual uint64_t rankZero(std::size_t pos) const { return pos - rank1(pos); }
