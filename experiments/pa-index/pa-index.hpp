...
#include "prefix_array_binary_search.hpp" // binary search ds
...
private:
	...
	prefix_array_binary_search<> S; // pa array binary search
...
public:
	...
	void build(const std::string &text_filepath, const std::string &pa_filepath, size_t refLen)
	{
						 ...
						 locate_pattern(const std::string pattern) const
	{
		...
		usafe_t b = S.locate_lower_bound(pattern);
		usafe_t e = S.locate_upper_bound(pattern);

		std::chrono::duration<double> duration_mid = 
				std::chrono::high_resolution_clock::now() - start;

		//std::cout << "--> " << b << " " << e << std::endl;
		//std::cout << "--> " << S[b-1] << " " << S[b] << " " << S[e-1] << " " << S[e] << " " << S[e+1] << std::endl;
		std::vector<uint_t> res = S.get_SA_range(b,e);

		//std::cout << res[0] << std::endl;

		/*
		std::cout << "-->" << S.locate_lower_bound(pattern) << std::endl;
		std::cout << "-->" << S.locate_upper_bound(pattern) << std::endl;
		exit(1);
		*/
	...
	void locate_fasta_test_running_time(const std::string patternFile) 
	{
		...
		while(std::getline(patterns, line))
		{
			if(i%2 != 0)
			{
				...
				//std::cout << std::get<0>(o)[0] << " " << std::get<0>(o)[std::get<0>(o).size()-1] << std::endl;
