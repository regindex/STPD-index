...
template <util::AllocType AT = util::AllocType::MALLOC> class EliasFano : public Rank, public Select {
  ...
  EliasFano(){ }
	 ...
	 * @param num_bits the length (in bits) of the bit vector.
	 */
	void build(const std::vector<uint64_t> ones, const uint64_t num_bits) {
		num_ones = ones.size();
		this->num_bits = num_bits;
		l = num_ones == 0 ? 0 : max(0, lambda_safe(num_bits / num_ones));
	...
	std::pair<uint64_t,uint64_t> successor_rank(uint64_t i) const
	{
		...
		res.second = rank1(i)+1;
		res.first = select1(res.second-1);
		return res;
	}
	...
	uint64_t serialize(std::ostream& out)
	{
		...
		out.write((char*)&block_length, sizeof(block_length));
		out.write((char*)&block_size_mask, sizeof(block_size_mask));
		out.write((char*)&lower_l_bits_mask, sizeof(lower_l_bits_mask));
		out.write((char*)&ones_step_l, sizeof(ones_step_l));
		out.write((char*)&msbs_step_l, sizeof(msbs_step_l));
		out.write((char*)&compressor, sizeof(compressor));

		w_bytes += sizeof(num_bits) + sizeof(num_ones) + sizeof(l) 
		         + sizeof(block_size) + sizeof(block_length) + sizeof(block_size_mask)
		         + sizeof(block_size_mask) + sizeof(lower_l_bits_mask) + sizeof(ones_step_l)
		         + sizeof(msbs_step_l) + sizeof(compressor);
	...
	{
		in.read((char*)&num_bits, sizeof(num_bits));
		in.read((char*)&num_ones, sizeof(num_ones));
		in.read((char*)&l, sizeof(l));
		in.read((char*)&block_size, sizeof(block_size));
		in.read((char*)&block_length, sizeof(block_length));
		in.read((char*)&block_size_mask, sizeof(block_size_mask));
		in.read((char*)&lower_l_bits_mask, sizeof(lower_l_bits_mask));
		in.read((char*)&ones_step_l, sizeof(ones_step_l));
		in.read((char*)&msbs_step_l, sizeof(msbs_step_l));
		in.read((char*)&compressor, sizeof(compressor));
