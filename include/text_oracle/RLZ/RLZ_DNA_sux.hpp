...
struct RLZ_DNA_sux { 
    ...
    void build_search_prefix( const std::string& input_filename, double epsilon = 1.0, size_t __prefix_len = 0 ) {
    ...
    void build( const std::string& input_filename, size_t prefix_len ) {
        sdsl::int_vector<8> text;
        sdsl::load_vector_from_file(text, input_filename, 1);

        size_t text_len   = text.size();

        builder b(&text, prefix_len);
    ...
    size_t text_length() const { return total_length; }
